...
const SustainabilityProfileScreen = ({ navigation }) => {
        ...
        try {
            ...
            if (response.data.status === 'success') {
                ...
                setFormData({
                    ...
                    primaryEnergySources: profile.primaryEnergySources || [],
    ...
    const toggleEnergySource = (source) => {
        setFormData(prev => {
            const isSelected = prev.primaryEnergySources.includes(source);
            const updatedSources = isSelected
                ? prev.primaryEnergySources.filter(s => s !== source)
                : [...prev.primaryEnergySources, source];
            
            return { ...prev, primaryEnergySources: updatedSources };
        });
    };

    const handleSave = async () => {
        if (!formData.primaryWaterSources.length || !formData.primaryEnergySources.length || formData.separateWaste === null) {
        ...
        try {
            ...
            const payload = {
                ...
                primaryEnergySources: formData.primaryEnergySources,
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            >
                {/* Primary Energy Sources Section */}
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <Ionicons name="flash-outline" size={24} color="#e36414" />
                        <Text style={styles.sectionTitle}>Primary Energy Sources</Text>
                    </View>
                        ...
                        {energySources.map((source, index) => (
                            <TouchableOpacity
                                ...
                                style={{
                                    ...
                                    alignItems: 'center',
                                    padding: 15,
                                    backgroundColor: formData.primaryEnergySources.includes(source) 
                                        ? '#FFF3E0' 
                                        : '#F8F9FA',
                                    borderWidth: 2,
                                    borderColor: formData.primaryEnergySources.includes(source)
                                        ? '#e36414'
                                        : '#E9ECEF',
                                    borderRadius: 12,
                                }}
                                onPress={() => toggleEnergySource(source)}
                            >
                                <View style={{
                                    width: 22,
                                    height: 22,
                                    ...
                                    {formData.primaryEnergySources.includes(source) && (
                                        <Ionicons name="checkmark" size={14} color="#fff" />
                                    )}
                                </View>
                                <Text style={{
                                    fontSize: 16,
                                    color: formData.primaryEnergySources.includes(source)
                                        ? '#2C3E50'
                                        : '#666',
                                    fontWeight: formData.primaryEnergySources.includes(source)
                                        ? '500'
                                        : '400'
                                }}>
                                    {source}
                ...
                <View style={styles.card}>
                    <View style={styles.cardHeader}>
                        <Ionicons name="trash-outline" size={24} color="#388e3c" />
                ...
                <View style={{ paddingHorizontal: 20, marginTop: 10, gap: 10 }}>
                    <TouchableOpacity
                        style={[
                            ...
                            (saving || !formData.primaryWaterSources.length || !formData.primaryEnergySources.length || formData.separateWaste === null) && 
                            styles.buttonDisabled
                        ]}
                        onPress={handleSave}
                        disabled={saving || !formData.primaryWaterSources.length || !formData.primaryEnergySources.length || formData.separateWaste === null}
                    >
